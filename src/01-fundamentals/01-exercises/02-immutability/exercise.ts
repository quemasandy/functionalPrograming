 ...
 */
function cambiarNombre(usuario: UsuarioInmutable, nuevoNombre: string): UsuarioInmutable {
 ...
 */
function cambiarCiudad(usuario: UsuarioInmutable, nuevaCiudad: string): UsuarioInmutable {
  return {
    ...usuario,
    direccion: {
 ...
 */
function actualizarEnIndice<T>(array: readonly T[], indice: number, nuevoValor: T): T[] {
 ...
 */
function eliminarEnIndice<T>(array: readonly T[], indice: number): T[] {
  return [
 ...
 */
function insertarEnIndice<T>(array: readonly T[], indice: number, valor: T): T[] {
  return [
    ...array.slice(0, indice), // Elementos antes
...
function agregarAlCarrito(carrito: Carrito, nuevoItem: ItemCarrito): Carrito {
  // Tu código aquí
  // Pistas:
 ...
 */
function actualizarCantidad(carrito: Carrito, productoId: string, nuevaCantidad: number): Carrito {
  // Mapeamos los items, cambiando solo el que coincide con productoId
  const nuevosItems = carrito.items.map(
    item =>
  ...
  const nuevoTotal = nuevosItems.reduce((sum, item) => sum + item.cantidad * item.precio, 0);
 ...
 */
function eliminarDelCarrito(carrito: Carrito, productoId: string): Carrito {
  // Filtramos para excluir el item con el productoId dado
  const nuevosItems = carrito.items.filter(item => item.productoId !== productoId);
